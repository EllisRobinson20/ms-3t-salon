...
export default function Calendar(props) {
  ...
  // functions - dynamic components in this file
  const renderHeader = () => {
  ...
  const renderCells = () => {
    ...
    const businessClosed = day => {
      return (
        ...
        dateFns.getDay(day) === 1 ||
        dateFns.getDay(day) === 2
      )
    }
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
  ...
  const onDateClick = day => {
    if (dateFns.format(day, 'eee') != 'Sun' && selectedService) {
      ...
      props.action(day)
    } else if (dateFns.format(day, 'eee') != 'Sun' && !selectedService) {
      setError('service')
    } else if (dateFns.format(day, 'eee') === 'Sun') {
      setError('sunday')
    }
  }
  const nextMonth = () => {
